class Car {
    constructor(x, y, width, height, controlType, maxSpeed = 3) {
        ...
        this.angle = 0;
        this.damaged = false;
        this.fitness = 0; // Track how far it's gone

        this.useBrain = controlType === "AI";

        if (controlType !== "DUMMY") {
            this.sensor = new Sensor(this);
            // Deeper network: sensors -> 12 -> 8 -> 4 outputs
    ...
    update(roadBorders, traffic) {
        if (!this.damaged) {
            ...
            // Fitness = how far up the road (lower y = further = better)
            if (!this.damaged) {
                this.fitness = -this.y;
            }
        }
    ...
    draw(ctx, color, drawSensor = false) {
        ...
        // Draw a small highlight on non-damaged cars
        if (!this.damaged) {
